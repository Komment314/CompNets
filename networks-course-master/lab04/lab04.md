...
3. На какой IP-адрес отправлен DNS-запрос? Используйте ipconfig для определения IP-адреса
   ...
   - Через ipconfig получил *81.89.176.30*
...
Повторите все шаги по предварительной подготовке из Задания B, но теперь для команды `nslookup –type=NS www.spbu.ru`
...
3. Проанализируйте ответное сообщение DNS. Имена каких DNS-серверов университета в
   ...
   - *www.spbu.ru: type CNAME, class IN, cname spbu.ru*, *spbu.ru: type NS, class IN, ns ns.pu.ru*, *spbu.ru: type NS, class IN, ns ns2.pu.ru*
   - Нет
